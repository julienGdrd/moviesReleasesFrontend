...
function Home() {
  ...
  console.log('listmovie out useeffect:',listMovies)
  ...
  const movies = listMovies.map((data, i) => {
    ...
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={`${data.overview.substring(0,100)} [...]`} poster={`https://image.tmdb.org/t/p/w500${data.poster_path}`} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
